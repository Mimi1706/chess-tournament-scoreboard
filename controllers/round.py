...
class RoundController:
    ...
    def create_round(self):
        ...
        for round in tournament["rounds_list"]: 
            for match in round["matches"]:
                if match["winner"] == "En cours":
                    self.view.custom_print("Vous devez d'abord entrer les scores des matchs du round en cours avant de pouvoir créer un nouveau round.")
        ...
        if tournament["current_round"] < tournament["rounds"]:
            ...
            round = RoundModel(next_round_number,created_matches, start_time=self.current_time, end_time="En cours")
            db_tournaments.update({"current_round": next_round_number}, doc_ids=[self.tournament_doc_id])
            db_tournaments.update(add("rounds_list", [round.serializer()]), doc_ids=[self.tournament_doc_id])
            self.view.custom_print("Round créé.")
        else: self.view.custom_print("Vous avez atteint le nombre maximal de rounds pour ce tournoi.")
    ...
    def update_round(self):
        ...
        for round in self.tournament["rounds_list"]:
            if round["round_number"] == int(current_round):
                round["start_time"] = self.view.custom_input("Horaire de début :")
                round["end_time"] = self.view.custom_input("Horaire de fin :")
